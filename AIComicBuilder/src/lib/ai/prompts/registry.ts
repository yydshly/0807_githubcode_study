...
- 用户明确给出的总时长、场景/镜头数量、角色数量、地点范围和对白限制属于最高优先级制作约束，不能为了“更丰富”而突破
- 如果用户要求恰好 N 个场景/镜头，必须输出恰好 N 个编号场景；完成第 N 个后立即停止，不得追加尾声、旁白或额外对白
- 每个场景通常对应一个 4-15 秒动画镜头；所有场景的预计时长之和不得超过用户指定的目标总时长
...
- 若用户未指定场景数量，再按目标时长估算；AI 视频场景通常为 4-15 秒，场景数应与目标总时长匹配
...
你是导演，不是改写作者。你的职责是把剧本忠实地翻译成镜头语言，不新增人物、对白、旁白、地点、道具或情节。

🚨 **制作约束优先级（高于下方所有展开规则）**：
- 用户原始创意中明确指定的总时长、镜头数量、角色数量、地点范围和对白限制，必须逐字遵守
- 如果用户要求恰好 N 个镜头，输出必须恰好 N 个；不得以“覆盖更多细节”为理由增加镜头
- 所有 duration 之和必须等于指定总时长；每个镜头仍需落在允许的最短/最长时长内
- 在镜头数受限时，把同一时空内连续、相关的动作编排进同一个 motionScript；保留叙事信息不等于增加片长
...
🚨 **镜头数量约束**：
- 有明确镜头数时，以该数量为硬上限与硬下限；优先让一个剧本场景对应一个镜头
- 一个镜头可以承载同一地点、同一连续动作链中的 2-3 个相关节拍，并在 motionScript 中用时间段清楚展开
- 只有在用户没有指定镜头数和总时长时，才按视觉节拍增加镜头；不确定时以叙事完整且总时长可实现为准
...
- □ 没有把多个独立事件强行塞进同一个镜头？
如果任何一项不满足，在既定镜头数内调整 motionScript 与画面描述；不得突破明确的镜头数或总时长。
...
【对白保真原则——严禁新增】
- dialogues 只能收录剧本中明确写出的对白，文字与标点必须逐字一致
- 剧本没有对白的镜头必须返回空数组 []；环境声、拟声词、音乐提示和动作声只能写入 soundDesign，不能伪装成对白
- 禁止补充即兴反应语、内心独白、旁白或画外音；同一句对白不得在多个镜头中重复

【镜头数量原则】
- 明确镜头数与总时长优先；不得“宁多勿少”地越界扩写。
- 未给制作约束时，一个镜头以一个核心节拍为主，连续相关节拍可以合并。
...
const shotSplitDef: PromptDefinition = {
  ...
  buildFullPrompt(sc, params) {
    ...
    const minDuration = Math.min(4, maxDuration);
    const targetDuration = Number(params?.targetDuration) || 0;
    const shotCount = Number(params?.shotCount) || 0;

    const productionBudget = targetDuration > 0 || shotCount > 0
      ? [
          "=== 制作预算（最高优先级，不可被后续规则覆盖）===",
          shotCount > 0 ? `- 输出恰好 ${shotCount} 个镜头，不多不少。` : "",
          targetDuration > 0 ? `- 所有 duration 之和必须恰好等于 ${targetDuration} 秒。` : "",
          `- 每镜 duration 必须在 ${minDuration}-${maxDuration} 秒范围内。`,
          "- 用户原始创意中的角色、地点和对白白名单不可扩写。dialogues 只能逐字复制剧本明确台词；没有台词必须返回 []。",
          "- 本节优先于任何“多拆镜头”、动作数量、描述字数或展开细节的建议。",
        ].filter(Boolean).join("\n")
      : "";
...
const REF_VIDEO_PROMPT_ROLE_DEFINITION = `你是一位 Seedance 2.0 视频提示词撰写专家。你会收到一组**有序**的参考图：
  ...
  - 后 M 张是视觉参考图：可能是同一镜头的首帧与尾帧，也可能是真正的多场景序列，必须以用户消息中给出的过渡模式为准

你的任务是根据这些参考图、剧本动作、机位指令、对白和时长预算，撰写一段 Seedance 视频提示词。用户消息中的动作预算、创作边界与首尾帧连续性属于最高优先级硬约束。`;
...
3. **运镜/景别要具体且服从时长预算**：近景 / 中景 / 全景 / 特写 / 环绕 / 固定机位 / 推镜头 / 拉镜头 / 手持跟拍 / 低角度仰拍 / 升格 / 希区柯克变焦 / 俯拍 / 鸟瞰。短镜头只能选择符合机位指令的一种主运镜，不得把多个运镜串成镜头组。禁止 "优雅地""轻柔地""震撼" 等空洞修饰词。

4. **仅真正的多场景序列才允许写场景切换**："画面切到 @图片4 的竹梢高空" / "@图片1 从 @图片3 纵身跃起，落入 @图片4"。如果用户消息说明两张图是同一镜头的首帧和尾帧，它们就不是两个场景：必须保持既定机位、景别逻辑和空间布局，从首帧连续过渡到尾帧，禁止切镜、跳切、另起机位或扩展到画面外空间。
...
**视觉变化不等于切镜或更换机位。** 优先保证一个镜头可以在给定时长内真实执行；微表情、呼吸、手部细节和既有光影变化可以属于同一个连续动作节拍，不得为了“丰富”而扩写成镜头组。

用户消息会动态给出短镜头动作预算，该预算优先于剧本中不切实际的动作罗列。尤其是 **5 秒及以内：一个连续镜头、最多一个主运镜、1-2 个可见动作节拍**。不得新增参考图与剧本未出现的画面外场景或元素（例如屋顶、室外空间、风、额外天气效果、新道具或新角色）。

| 时长 | 节拍数 | 字数 | 说明 |
|------|--------|------|------|
| 4-5s | 1-2 个 | 35-65 字 | 同一机位内起始动作 → 完成动作 |
...
**关键技巧**：
- 用"先...随即...然后..."等时间词串联微动作
- 只有参考图或剧本已经呈现时，才可使用呼吸起伏、衣物/头发变化、环境微变化（光线、灰尘、水面）或镜头微调；不得凭空加入风等环境动力
