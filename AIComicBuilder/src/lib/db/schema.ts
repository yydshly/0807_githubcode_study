...
/**
 * Durable commands issued by Codex (or another local orchestrator).
 *
 * Provider credentials deliberately do not live here. `providerProfileId`
 * points at a server-side profile, while payload contains only generation
 * inputs that are safe to persist.
 */
export const orchestratorRuns = sqliteTable(
  ...
  (table) => [
    uniqueIndex("orchestrator_runs_user_idempotency_key_unique").on(
      table.userId,
      table.idempotencyKey,
    ),
    index("orchestrator_runs_status_idx").on(table.status),
    index("orchestrator_runs_project_created_idx").on(
      table.projectId,
      table.createdAt,
    ),
  ],
