...
import { toStrictTextStreamResponse } from "@/lib/ai/strict-text-stream-response";
...
import { resolveRequestModelConfig } from "@/lib/ai/server-profiles";
...
export async function POST(
  ...
  const { action, payload, modelConfig: clientModelConfig, providerProfileId, episodeId } = body;
  let modelConfig: ModelConfig | undefined;
  try {
    modelConfig = resolveRequestModelConfig(clientModelConfig, providerProfileId);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Invalid provider profile";
  ...
  if (action === "shot_split") {
    return handleShotSplitStream(projectId, userId, payload, modelConfig, episodeId);
  }
...
async function handleScriptOutlineAction(
  ...
  return toStrictTextStreamResponse(result.fullStream, {
    onSuccess: async (text) => {
      const outline = text.trim();
      if (!outline) throw new Error("Outline generation returned empty text");
      if (episodeId) {
        await db
          .update(episodes)
...
async function handleScriptGenerate(
  ...
  return toStrictTextStreamResponse(result.fullStream, {
    onSuccess: async (text) => {
      if (!text.trim()) throw new Error("Script generation returned empty text");
      if (episodeId) {
        await db
          .update(episodes)
...
async function handleScriptParseStream(
  ...
  return toStrictTextStreamResponse(result.fullStream, {
    onSuccess: async (text) => {
      ...
      JSON.parse(screenplay); // validate JSON before reporting success
      if (episodeId) {
        await db.update(episodes).set({ updatedAt: new Date() }).where(eq(episodes.id, episodeId));
      } else {
        await db.update(projects).set({ updatedAt: new Date() }).where(eq(projects.id, projectId));
...
`;
  ...
  const payloadShotCount = Number(payload?.shotCount);
  const requestedShotCount = Number.isInteger(payloadShotCount) && payloadShotCount > 0
    ? payloadShotCount
    : extractRequestedShotCount(productionBrief);
  const plannedDurations = requestedShotCount && targetDuration > 0
    ? buildEvenDurationPlan(targetDuration, requestedShotCount, Math.min(4, videoMaxDuration), videoMaxDuration)
    : null;

  if (requestedShotCount && targetDuration > 0 && !plannedDurations) {
    return NextResponse.json(
      {
        error: `制作约束不可同时满足：${requestedShotCount} 个镜头无法在每镜 4-${videoMaxDuration} 秒范围内组成 ${targetDuration} 秒`,
      },
      { status: 422 }
    );
  }
  ...
  // Exact short-form budgets must be solved as one whole. Splitting them would
  // incorrectly ask every chunk to independently consume the full runtime.
  const sceneChunks = requestedShotCount && requestedShotCount <= 16
    ? [fullScript]
    : splitScriptByScenes(fullScript, 8);
  ...
  const chunkResults = await Promise.all(
    sceneChunks.map(async (chunk, idx) => {
      ...
      if (productionBrief) {
        prompt = `【用户原始制作要求（最高优先级）】\n${productionBrief}\n\n不得突破其中的时长、镜头数、角色、地点或对白限制。\n\n` + prompt;
      }
      ...
      if (targetDuration && targetDuration > 0) {
        prompt += `\n\n目标总时长：${targetDuration}秒（${Math.floor(targetDuration / 60)}分${targetDuration % 60}秒）。所有 duration 之和必须恰好等于此目标。\n`;
      }
      if (requestedShotCount) {
        prompt += `\n【不可违背的镜头预算】只输出恰好 ${requestedShotCount} 个镜头，不多不少。`;
        if (sceneMatches.length === requestedShotCount) {
          prompt += ` 剧本有 ${sceneMatches.length} 个编号场景，每个场景恰好对应一个镜头。`;
        }
        if (plannedDurations) {
          prompt += ` 第 1-${requestedShotCount} 镜的 duration 依次必须为 [${plannedDurations.join(", ")}] 秒，总和恰好 ${targetDuration} 秒。`;
        }
        prompt += ` 没有明确台词的镜头，dialogues 必须是 []；音效、拟声词、沉默和省略号都不是台词。\n`;
      }
      try {
  ...
  if (requestedShotCount && allShots.length !== requestedShotCount) {
    return NextResponse.json(
      {
        error: `分镜质量门未通过：要求恰好 ${requestedShotCount} 镜，模型返回 ${allShots.length} 镜；本次结果未保存`,
      },
      { status: 422 }
    );
  }

  const allowedDialogueCounts = extractScreenplayDialogueCounts(fullScript);
  const usedDialogueCounts = new Map<string, number>();
  for (const shot of allShots) {
    for (const dialogue of shot.dialogues || []) {
      const text = dialogue.text?.trim();
      const allowed = text ? (allowedDialogueCounts.get(text) ?? 0) : 0;
      const used = text ? (usedDialogueCounts.get(text) ?? 0) : 0;
      if (!text || used >= allowed) {
        return NextResponse.json(
          {
            error: `分镜质量门未通过：检测到剧本原文之外或重复的对白“${text || "（空）"}”；本次结果未保存`,
          },
          { status: 422 }
        );
      }
      usedDialogueCounts.set(text, used + 1);
    }
  }

  if (plannedDurations) {
    allShots.forEach((shot, index) => {
      shot.duration = plannedDurations[index];
    });
  }
...
function extractRequestedShotCount(brief: string): number | undefined {
  const patterns = [
    /(?:恰好|正好|严格|总共|共|约)?\s*(\d{1,3})\s*个?\s*(?:镜头|分镜)/i,
    /(?:镜头|分镜)(?:数量|数)?\s*[：:=]?\s*(\d{1,3})/i,
    /\b(?:exactly|about|around)?\s*(\d{1,3})\s+shots?\b/i,
  ];
...
function buildEvenDurationPlan(
  ...
  const longest = base + (remainder > 0 ? 1 : 0);
  if (base < minDuration || longest > maxDuration) return null;
  return Array.from(
    { length: shotCount },
...
function extractScreenplayDialogueCounts(screenplay: string): Map<string, number> {
...
async function handleSingleReferenceVideo(
  ...
  try {
    ...
    const sceneAssetList = shotView.referenceImages
      .filter((r) => r.fileUrl)
      .sort((a, b) => a.sequenceInType - b.sequenceInType);
    const sceneReferenceInputs = sceneFramePaths.map((image, i) => {
    ...
    // This ordered result is the single source of truth for both @图片N
    // labels and the actual multimodal/video reference payload.
    const orderedReferences = buildOrderedRefVideoPromptReferences({
      characters: charRefs.map((c) => ({
        name: c.name,
        image: c.imagePath,
        visualHint: projectCharacters.find((pc) => pc.name === c.name)?.visualHint,
      })),
      sceneFrames: sceneReferenceInputs,
    });
    const {
      images: orderedRefImages,
      characters: characterRefInfos,
      sceneFrames: sceneFrameInfos,
      sceneImages: orderedSceneImages,
    } = orderedReferences;
    if (orderedSceneImages.length === 0) {
      throw new Error("No valid scene reference images. Please generate scene reference images first.");
    }
    const fullMapping = [
    ...
    const result = await videoProvider.generateVideo({
      initialImage: orderedSceneImages[0],
      prompt: videoPrompt,
      duration: effectiveDuration,
      ratio,
  ...
  const results = await Promise.all(
    eligible.map(async (shot): Promise<{ shotId: string; sequence: number; status: "ok" | "error"; referenceVideoUrl?: string; error?: string }> => {
      try {
        ...
        // Step 2: Build one ordered source for both labels and actual images.
        const orderedReferences = buildOrderedRefVideoPromptReferences({
          characters: charRefs.map((c) => ({
            name: c.name,
            image: c.imagePath,
        ...
        if (orderedSceneImages.length === 0) {
          throw new Error("No valid scene reference images. Generate scene reference images first.");
        }
        const fullMapping = [
          ...characterRefInfos.map((c) => `@图片${c.index}是${c.name}`),
          ...sceneFrameInfos.map((s) => `@图片${s.index}是${s.label}`),
...
async function handleSingleVideoPrompt(
  ...
  try {
    ...
    const hasKeyframePair =
      genMode !== "reference" && Boolean(shotView.firstFrame && shotView.lastFrame);
    const sceneReferenceInputs = visionFrames.map((image, i) => {
      const name = hasKeyframePair
        ? (i === 0 ? "首帧" : "尾帧")
        : sceneMetaList[i]?.sceneName || (visionFrames.length > 1 ? `场景-${i + 1}` : `场景`);
      return { label: name, image };
    });
    const orderedReferences = buildOrderedRefVideoPromptReferences({
      characters: charsWithRefsHere.map((c) => ({
        name: c.name,
    ...
    if (sceneFrameInfos.length === 0) {
      return NextResponse.json({ error: "No valid frame available. Generate frames first." }, { status: 400 });
    }
    const promptRequest = buildRefVideoPromptRequest({
      motionScript: motionContext,
      cameraDirection: shot.cameraDirection || "static",
      ...
      characters: characterRefInfos,
      sceneFrames: sceneFrameInfos,
      transitionMode: hasKeyframePair ? "keyframe-pair" : "scene-sequence",
      dialogues: dialogueList.length > 0 ? dialogueList : undefined,
    });
  ...
  const results = await Promise.all(
    eligible.map(async (shot) => {
      try {
        ...
        const hasKeyframePair =
          batchGenMode !== "reference" && Boolean(shotLegacy?.firstFrame && shotLegacy?.lastFrame);
        const sceneReferenceInputs = visionFrames.map((image, i) => {
          const name = hasKeyframePair
            ? (i === 0 ? "首帧" : "尾帧")
            : sceneMetaList[i]?.sceneName || (visionFrames.length > 1 ? `场景-${i + 1}` : `场景`);
        ...
        const orderedReferences = buildOrderedRefVideoPromptReferences({
          characters: batchCharsWithRefs.map((c) => ({
            name: c.name,
            image: c.referenceImage,
            visualHint: c.visualHint,
          })),
        ...
        if (sceneFrameInfos.length === 0) {
          throw new Error("No valid frame available. Generate frames first.");
        }
        const promptRequest = buildRefVideoPromptRequest({
          motionScript: motionContext,
          cameraDirection: shot.cameraDirection || "static",
